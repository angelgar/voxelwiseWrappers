...
for (i in 1:length(opt)){
  if (is.na(opt)[i] == T) {
    ...
    cat('Use gamm4ROI.R -h for an expanded usage menu.\n')
    quit()
  }
}
...
m <- mclapply(model.formula, function(x) {
  ...
  foo <- gamm4(formula = x, random=as.formula(randomFormula), data=dataSubj, REML=T)$gam
  summary <- summary(foo)
...
if (is.null(m[[1]]$s.table)) {
  ...
  write.csv(p.output, paste0(OutDirRoot, "_coefficients.csv"), row.names=F)
